...
export default function BodyMap({ onAnalyze }) {
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      {/* Kirjauslomake ylhäällä */}
      <div style={{ marginBottom: '24px' }} className="flex flex-col gap-4">
                ...
                {alueet.map(alue => {
                  ...
                  return (
                    <button
                      ...
                      style={löydös && !isActive ? { borderColor: kipuVäri(löydös.kipu) } : {}}
                    >
                      {alue.nimi}
                      {löydös && (
                        <span className="ml-1.5 text-xs opacity-70">VAS {löydös.kipu}</span>
                      )}
                    </button>
                  )
                })}
        ...
        {valittu && valittuData && valittuMääritys ? (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div className="flex items-start justify-between mb-1">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Mittaa: {valittuMääritys.mittaus}</p>
              </div>
              <button
          ...
          <div className="bg-gray-50 rounded-xl border border-dashed border-gray-200 p-6 text-center">
            <p className="text-sm text-gray-400">Valitse alue yllä kirjataksesi löydöksen.</p>
          </div>
        )}
        ...
        {löydösMäärä > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <ul className="divide-y divide-gray-50 -mx-2 mb-4">
              {KIRJAUSRAKENNE.filter(a => löydökset[a.id]).map(alue => {
                const data     = löydökset[alue.id]
                const isActive = alue.id === valittu
                const rInfo    = ryhmäInfo(alue.tyyppi)
                const kirjatut = Object.entries(data.kirjaukset).filter(([, v]) => v !== null)
                return (
                  <li
                    key={alue.id}
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-sm font-medium text-gray-700 truncate">{alue.nimi}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs ml-2 flex-shrink-0">
                      {kirjatut.map(([kid, v]) => (
                        <span key={kid} className="bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">{v}</span>
                      ))}
      ...
      {/* Kehokuva koko leveydellä alhaalla */}
