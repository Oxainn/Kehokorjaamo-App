...
function TilaOdottaa({ findings, onKopioi }) {
...
function TilaKopioitu({ vastaus, onVastausChange, onKäytä, onPalaa }) {
  return (
    <div className="flex flex-col gap-4">
      <div className="bg-brand-50 border border-brand-100 rounded-xl p-4 flex items-center gap-3">
        <span className="text-brand-600 text-lg font-bold">✓</span>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <ol className="space-y-2 text-sm text-gray-600 mb-5">
          <li className="flex gap-2">
            <span className="font-semibold text-brand-600 w-4 flex-shrink-0">1.</span>
            Avaa <span className="font-medium text-gray-800 ml-1">Claude.ai</span>
          </li>
          <li className="flex gap-2">
            <span className="font-semibold text-brand-600 w-4 flex-shrink-0">2.</span>
            Liitä prompt (<kbd className="bg-gray-100 px-1.5 py-0.5 rounded text-xs font-mono">Ctrl+V</kbd>)
          </li>
          <li className="flex gap-2">
            <span className="font-semibold text-brand-600 w-4 flex-shrink-0">3.</span>
            Kopioi Clauden vastaus
          </li>
          <li className="flex gap-2">
            <span className="font-semibold text-brand-600 w-4 flex-shrink-0">4.</span>
        ...
        <textarea
          value={vastaus}
          onChange={(e) => onVastausChange(e.target.value)}
          rows={8}
...
function TilaTulos({ tulos, onPalaa }) {
  return (
    <div className="flex flex-col gap-4">
...
export default function TreatmentPlan({ findings = [], onResult }) {
  ...
  return (
    <section className="flex flex-col gap-6">
      ...
      {vaihe === 'odottaa' && (
        <TilaOdottaa findings={findings} onKopioi={kopioi} />
      )}

      {vaihe === 'kopioitu' && (
        <TilaKopioitu
          vastaus={vastaus}
          onVastausChange={setVastaus}
          onKäytä={käytäVastausta}
          onPalaa={nollaa}
        />
      )}

      {vaihe === 'tulos' && tulos && (
        <TilaTulos tulos={tulos} onPalaa={nollaa} />
      )}
    </section>
  )
}
